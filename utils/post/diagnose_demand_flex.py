"""Per-utility demand-flex elasticity diagnostic for NY heat-pump rate design.
...
ARCTURUS_INTERCEPT = -0.011
ARCTURUS_SLOPE = -0.065
...
def arcturus_peak_reduction(price_ratio: float) -> float:
    ...
    if price_ratio <= 1.0:
        return 0.0
    return abs(ARCTURUS_INTERCEPT + ARCTURUS_SLOPE * math.log(price_ratio))
...
def diagnose_season(
    ...
    time_idx = pd.DatetimeIndex(loads_df.index.get_level_values("time").unique())
    month_arr = time_idx.month
    ...
    season_mask = month_level.isin(season_months)
    ...
    peak_rate = season_spec.peak_offpeak_ratio * season_spec.base_rate
    offpeak_rate = season_spec.base_rate
...
def diagnose_utility(
    ...
    result = UtilityResult(utility=ctx.utility)
...
def print_summary(results: list[UtilityResult]) -> None:
    """Print a human-readable summary to stdout."""
    print("\n" + "=" * 80)
    print("DEMAND-FLEX ELASTICITY DIAGNOSTIC")
    print("=" * 80)
    ...
    print("  - Single annual elasticity is a compromise across seasons.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    elasticities = [float(e) for e in args.elasticities.split(",")]
    utilities = (
    ...
    for utility in utilities:
        ...
        hp_weights = dict(
            zip(util_hp["bldg_id"].to_list(), util_hp["weight"].to_list())
        )
        hp_weighted = float(util_hp["weight"].sum())

        util_all = all_meta.filter(pl.col("sb.electric_utility") == utility)
        total_weighted = float(util_all["weight"].sum())
        ...
        specs, tou_tariff = load_tou_context(utility, tou_deriv_dir, tariffs_dir)
        log.info(
            "  TOU specs: %s",
