...
def _convert_tou_matrix_to_urdb(
    ...
    for row in tou_mat:
        ...
        entry: dict[str, Any] = {"rate": rate, "adj": adj, "unit": unit_map[units]}
        if not math.isclose(max_usage, 1e38, rel_tol=0.0, abs_tol=1e-6):
            entry["max"] = max_usage
...
def _convert_cairo_payload_to_urdb(
    ...
    *,
    ...
    if tariff_key in payload and isinstance(payload[tariff_key], dict):
        cairo_tariff = payload[tariff_key]
    elif len(payload) == 1:
        only_value = next(iter(payload.values()))
        if not isinstance(only_value, dict):
            raise ValueError("Unexpected tariff_final_config.json structure")
        cairo_tariff = only_value
    else:
        raise ValueError(
