...
NY_SUPPLY_ENERGY_BASE = "s3://data.sb/switchbox/marginal_costs/ny/supply/energy/utility="
NY_SUPPLY_CAPACITY_BASE = "s3://data.sb/switchbox/marginal_costs/ny/supply/capacity/utility="
SUPPLY_MC_YEAR = "2025"
...
def main() -> None:
    ...
    for i, header in enumerate(headers, start=1):
        ...
        elif "tou" in header_lower and "supply" in header_lower and "mc" in header_lower:
    ...
    required = ["state", "utility", "num", "path_supply_energy_mc", "path_supply_capacity_mc"]
    ...
    # Column E is "supply" which indicates add_supply_revenue_requirement (X = true)
    supply_col = _get_column_letter(col_indices.get("supply", 5))  # E (default to 5 if not found)
    tou_mc_col_1 = _get_column_letter(col_indices.get("path_tou_supply_capacity_mc_1", 23))  # W
    tou_mc_col_2 = _get_column_letter(col_indices.get("path_tou_supply_capacity_mc_2", 24))  # X
    ...
    ws.update(values=[[f] for f in energy_formulas], range_name=energy_range, value_input_option="USER_ENTERED")
    ...
    ws.update(values=[[f] for f in capacity_formulas], range_name=capacity_range, value_input_option="USER_ENTERED")
    ...
    if "path_tou_supply_capacity_mc_1" in col_indices:
        ...
        ws.update(values=[[f] for f in tou_energy_formulas], range_name=tou_range_1, value_input_option="USER_ENTERED")
    ...
    if "path_tou_supply_capacity_mc_2" in col_indices:
        ...
        ws.update(values=[[f] for f in tou_capacity_formulas], range_name=tou_range_2, value_input_option="USER_ENTERED")
        print(f"✓ Updated {tou_mc_col_2} column (path_tou_supply_capacity_mc - capacity)")
