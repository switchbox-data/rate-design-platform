# Classifying NiMo MCOS projects by voltage tier

This directory contains scripts and data for classifying NiMo's MCOS (Marginal Cost of Service) study projects into three voltage tiers for the Bill Alignment Test:
...
NiMo's MCOS workbook lists ~237 planned capital projects, but most transmission projects are grouped under a generic "Transm Net" label with no voltage information. Without voltage, we can't separate bulk transmission costs (which customers already pay through NYISO charges) from sub-transmission costs (which belong in the local marginal cost used for BAT).
