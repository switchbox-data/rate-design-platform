...
try:
    from utils.eia_region_config import StateConfig, get_aws_storage_options, get_state_config
except ModuleNotFoundError:  # pragma: no cover - supports running from utils/ directory
    from eia_region_config import StateConfig, get_aws_storage_options, get_state_config
...
def transform_eia_data(
    ...
    df = df.with_columns([
        pl.col("subba").replace(zone_mapping).alias("zone")
    ])
...
def fetch_zone_data(
    ...
    """
    print(f"Fetching {config.label} zonal load data from EIA API ({start_date} to {end_date})...")
...
def generate_expected_hourly_timestamps(
    ...
    start_dt = datetime.strptime(start_date, "%Y-%m-%d").replace(tzinfo=ZoneInfo(timezone))
...
def validate_zone_data(
    ...
    Args:
        ...
        end_date: End date (YYYY-MM-DD)
    """
    expected_hours = len(generate_expected_hourly_timestamps(start_date, end_date, timezone))
...
def main():
    ...
    partitions = (
        df_with_partitions
        .select(["region", "zone", "year", "month"])
        .unique()
        .sort(["region", "zone", "year", "month"])
    )
    print(f"\nPreparing to upload {len(partitions)} partitions:")
    for row in partitions.iter_rows(named=True):
        region, zone, year, month = row["region"], row["zone"], row["year"], row["month"]
        ...
        print(f"  region={region}/zone={zone}/year={year}/month={month}/ - {count:,} rows")
