...
| \( T \)        | Set  | Time periods in billing month, \( t \in \{1, 2, ..., T\} \) where \( T ~= 2976 \) (15-min intervals) | -        | \|T\| × 1  |
...
U_{m,t}^{HW} \cdot \rho \cdot c_p \cdot (T_{m,t}^{setpoint} - T_{m,t}^{tank}) & \text{if } T_{m,t}^{tank} < T_{m,t}^{setpoint} \text{ and } U_{m,t}^{HW} > 0 \\
0 & \text{otherwise}
\end{cases}
\]
