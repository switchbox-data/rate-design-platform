...
Cost Allocation (PoP methodology)
    ↓
s3://data.sb/switchbox/marginal_costs/ny/region={iso_region}/utility={utility}/year={year}/data.parquet
```
...
**Path**: `s3://data.sb/switchbox/marginal_costs/ny/region={iso_region}/utility={utility}/year={year}/data.parquet`

**Schema**:

- `timestamp` (datetime): Hour ending timestamp
- `region` (string): ISO region partition key (e.g., nyiso)
- `utility` (string): Utility name
...
```
timestamp            region  utility  year  mc_total_per_kwh
2024-07-15 15:00:00  nyiso   NYSEG    2026  0.02110
2024-07-15 16:00:00  nyiso   NYSEG    2026  0.02201
2024-01-01 03:00:00  nyiso   NYSEG    2026  0.00000
...
```
...
python generate_utility_tx_dx_MC.py \
    --utility NYSEG \
    --year 2026 \
    --mc-table-path ../config/marginal_costs/ny_marginal_costs_2026_2035.csv
```
...
**Output**: `s3://data.sb/switchbox/marginal_costs/ny/region={iso_region}/utility={utility}/year={year}/data.parquet`
...
**Path**: `rate_design/ny/hp_rates/config/marginal_costs/ny_marginal_costs_2026_2035.csv`
...
mc_path = S3Path("s3://data.sb/switchbox/marginal_costs/ny/region=nyiso/utility=NYSEG/year=2026/data.parquet")
...
joined = customer_df.join(
    mc_df.select(["timestamp", "mc_total_per_kwh"]),
    on="timestamp",
    how="left"
)
