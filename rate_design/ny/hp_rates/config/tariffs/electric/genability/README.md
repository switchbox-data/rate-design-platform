# NY Genability tariff data

This directory contains Genability/Arcadia tariff snapshots and derived charge data
for all seven NY electric utilities: CenHud, ConEd, National Grid (NiMo), NYSEG,
O&R, PSEG-LI, and RG&E.
...
```bash
just -f rate_design/ny/hp_rates/config/tariffs/electric/Justfile fetch-genability 2025-01-01
```
...
```bash
just -f rate_design/ny/hp_rates/Justfile fetch-monthly-rates coned 2025-01 2025-12
```

Produces `coned_monthly_rates.yaml`.
...
2. **Add an entry to a `genability_tariffs.yaml`** in the state's tariff config directory (use `default` to get the residential default tariff, or a specific `masterTariffId`).
