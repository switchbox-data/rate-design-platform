...
Import this module at the top of run_scenario.py (after all other imports):
    import rate_design.ny.hp_rates.patches  # noqa: F401  (currently no-op; patches added below)
"""
...
def _return_loads_combined(
    ...
    # no-shift case (offset_hours=0 leaves data unchanged). For the current NY
...
def _vectorized_process_building_demand_by_period(
    ...
    Handles flat and time-of-use tariffs (covers all 12 NY runs, which are all flat or TOU
...
def _vectorized_run_system_revenues(
    ...
    Handles flat and TOU tariffs (all NY runs). Falls back to original CAIRO
