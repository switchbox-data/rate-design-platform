...
Input:
    - Utility 8760 load profile: s3://data.sb/nyiso/loads/utility=X/year=YYYY/data.parquet
...
def load_utility_load_profile(s3_base: str, year_load: int, utility: str) -> pl.DataFrame:
    ...
    Reads from Hive-style partitioned structure: utility=X/year=YYYY/data.parquet
    
    Args:
        s3_base: Base S3 path for NYISO loads (e.g., s3://data.sb/nyiso/loads)
    ...
    """
    # Use Hive-style partitioning: utility=X/year=YYYY/data.parquet
    load_path = S3Path(s3_base) / f"utility={utility}" / f"year={year_load}" / "data.parquet"
    
    if not load_path.exists():
        raise FileNotFoundError(f"Utility load profile not found: {load_path}")
    
    # Read from S3 using BytesIO buffer (uses boto3)
...
def normalize_load_to_cairo_8760(load_df: pl.DataFrame, utility: str, year_load: int) -> pl.DataFrame:
    ...
    if has_feb29:
        print("  Leap-year pattern detected (Feb 29 present); dropping Dec 31 to match Cairo.")
        df = df.filter(
            ~((pl.col("timestamp").dt.month() == 12) & (pl.col("timestamp").dt.day() == 31))
        )
    ...
    if duplicate_count > 0:
        print(f"  Collapsing {duplicate_count} duplicate timestamp row(s) with mean load.")
    ...
    # Final hard checks.
    if df.height != 8760:
        raise ValueError(f"Cairo 8760 normalization failed: expected 8760 rows, got {df.height}")
...
def save_allocated_costs(
    ...
    Output structure: utility=X/year=YYYY/data.parquet
    ...
    output_path = base_path / f"utility={utility}" / f"year={year}" / "data.parquet"
...
def main():
    ...
    load_df = load_utility_load_profile(args.nyiso_s3_base, load_year, args.utility)
    ...
    mc_upstream, mc_dist = get_marginal_costs_for_year(mc_df, args.utility, args.mc_year)
