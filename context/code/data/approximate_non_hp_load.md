...
**Recommended:** This step is orchestrated via `data/resstock/main.py` as step 2c-i (`_approximate_non_hp_load` function). It runs on local EBS after the raw-to-`_sb` clone and utility assignment, using `k=15` and `include_cooling=False`, once per upgrade in `approx_upgrade_ids`. See `context/code/data/resstock_sb_release_pipeline_main_py.md`.
