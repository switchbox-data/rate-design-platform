...
OCC names exactly two "primary uses" for the `$95.6M` Eversource SBC pool, and both are hardship/arrearage/low-income assistance programs — not a general public-benefits catch-all. Combined with earlier research (CGA legislative reports and the OCC RAM Overview, both showing CT's SBC is overwhelmingly, \>85%, devoted to hardship/energy-assistance programs), that supports **`exclude_eligibility`** rather than `add_to_drr` — this reads as an income-transfer program (RI LIDRF precedent), not a rate-design cost that should shape the BAT's measured cross-subsidy. Decision: **`exclude_eligibility`**, `master_charge`: "Systems Benefits Charge (SBC)", `master_type`: "LMI cost recovery". Caveat carried forward: OCC's opening phrase "public education... and other societal costs" implies a residual non-LMI component that isn't quantified anywhere found so far; if it's material, only part of the `$0.03326/kWh` should really be `exclude_eligibility` with the rest `add_to_drr`, but the whole charge is encoded as `exclude_eligibility` for now given the \>85% LMI share.
