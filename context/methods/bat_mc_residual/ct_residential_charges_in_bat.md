...
**Current modeling decision: all Eversource CT ResStock buildings are assigned to Rate 1's tariff, sized to the whole residential rate class.** Rate 5 (electric heating, closed since 2006) and Rate 7 (TOU) are small rate classes (135k and 731 customers respectively, vs. ~1.05M on Rate 1). Every building uses Rate 1's _tariff_, but the revenue-requirement target (base DRR, test-year kWh, customer count) is the **sum of Rate 1 + Rate 5 + Rate 7** — see [`ct_eversource_class_rev_requirement.md`](../../code/data/ct_eversource_class_rev_requirement.md). The class file is `ct_eversource_rate_case_test_year.yaml` (test year, current rates) with a rate-year-1 companion `ct_eversource_rate_case_rate_year_1.yaml` (Proposed+GET); the earlier Rate-1-only target is archived at `ct_eversource_rate1_rate_case_test_year.yaml`. Rate 5/7 seed YAMLs exist (`ct_eversource_elecheat_rate_case_test_year.yaml`, `ct_eversource_tou_rate_case_test_year.yaml`) with testimony values and feed the class sums, but the build script's ResStock lookup uses `ct_eversource` — the only utility assignment in the CT ResStock data. Splitting buildings by heating fuel to model Rate 5 separately, or offering Rate 7 as an alternative tariff, is future work.
...
**Known limitation on Rate 7 default tariff JSONs:** `create_default_structure_tariffs.py` also cannot parse the TOU `all_on_peak`/`all_off_peak` keys. The generated `ct_eversource_tou_default.json` and `ct_eversource_tou_default_supply.json` are **incorrect** — they contain only the flat charges (core delivery + ESI + CTA + CAM + RE ≈ `$0.081`/kWh) and silently drop transmission, NBFMCC, and supply (which are all TOU-structured). The correct all-in delivery rate should be ~`$0.15`–`$0.17`/kWh (comparable to Rate 1). These files should not be used without fixing the script's TOU handling or manually constructing the tariff. Not blocking because all CT buildings are modeled on Rate 1.
