...
- **Peak/off-peak price ratios** range from 1.57 (CenHud winter) to 4.33 (ConEd summer)
...
3. At each candidate elasticity (-0.02 through -0.30 in 0.02 steps), applies the same constant-elasticity shifting formula CAIRO uses: $Q_{\text{shifted}} = Q_{\text{orig}} \times (P_{\text{period}} / P_{\text{flat}})^\varepsilon$
...
1. **Uniform vs heterogeneous response**: Arcturus measures aggregate outcomes from diverse customers; our model applies the same epsilon to every HP building.
