...
order of magnitude. The platform currently makes *different* choices in different states, so this
...
Per the methodology doc §3, "LRMC is lower than average cost" is universal, but the *magnitude* of
...
2. **Average Incremental Cost (AIC) / avoided cost** (intermediate–large). The capital cost of a
   ...
   This is "cost per kW of peak" — what one more kW of peak *causes*, or equivalently what one kW
   of peak reduction *avoids*. The UK CDCM (500 MW increment), the CPUC Avoided Cost Calculator,
   the New England AESC avoided-T&D values, and utility "avoided distribution cost" studies all sit
   here.

3. **Forward-Looking Incremental Cost (FLIC)** (smallest). The capital of *specific planned
   projects* entering service in a near-term window, annualized and divided by **total system
   peak**:
...
All three feed the *same* PoP allocator
...
2. **Within the methodology doc itself.** [§3](../bat_mc_residual/bat_lrmc_residual_allocation_methodology.md)
   ...
   CPUC ACC figure. The `$54.46` is an avoided-cost (AIC) number an order of magnitude *above* NY's
...
consistent with BGE's *own* cost-of-service conception, which matters because we intend to intervene
...
- Every marginal/avoided number BGE itself publishes uses a **peak denominator**: Brattle divides
  replacement value by peak (`$32`); E3 divides by peak *change* (`$203–258`). Neither dilutes one
