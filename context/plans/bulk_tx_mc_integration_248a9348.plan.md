...
todos:
  ...
  - id: zone-mapping
    ...
    status: completed
  - id: generate-module
    content: Create utils/data_prep/marginal_costs/generate_bulk_tx_mc.py with SCR allocation
    status: completed
  - id: cairo-loader
...
| S3: `.../transmission/` | S3: `.../bulk_tx/` (output from `utils/data_prep/marginal_costs/generate_bulk_tx_mc.py`) |
...
flowchart TD
    ...
    VzTable --> GenModule["utils/data_prep/marginal_costs/generate_bulk_tx_mc.py"]
...
## 2. New module: `utils/data_prep/marginal_costs/generate_bulk_tx_mc.py`

Mirrors [`utils/data_prep/marginal_costs/generate_supply_capacity_mc.py`](utils/data_prep/marginal_costs/generate_supply_capacity_mc.py) pattern:

- **Inputs**: Zone mapping (same CSV), utility loads (same S3 base + load functions imported from [`generate_utility_tx_dx_mc.py`](utils/data_prep/marginal_costs/generate_utility_tx_dx_mc.py)), derived v_z table (CSV on S3 or local), year
...
- 1 kW constant load test: `sum(pi_t * 1 kW * 1 hour) == v_z` (same pattern as [`validate_allocation`](utils/data_prep/marginal_costs/supply_capacity.py) and [`validate_allocation`](utils/data_prep/marginal_costs/generate_utility_tx_dx_mc.py))
- Non-zero hours == 80
...
- `create-dist-and-sub-tx-mc-data` recipe: invokes `utils/data_prep/marginal_costs/generate_utility_tx_dx_mc.py`
...
- `create-bulk-tx-mc-data utility_arg` recipe: calls `utils/data_prep/marginal_costs/generate_bulk_tx_mc.py`
