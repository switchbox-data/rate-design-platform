...
- **`compare_resstock_eia861_loads.py`** (see `context/tools/compare_resstock_eia861_loads.md`): Simpler script; outputs a CSV of ResStock vs EIA totals and customer counts by utility (optionally with `--normalize-by-customer-count`). No MF adjustment, no SF/MF analysis.
