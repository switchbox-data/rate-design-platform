...
# File types excluded from cloning raw → _sb. These are fetched for the raw
# release but NOT copied to _sb via clone because _sb produces its own version
# from modified hourly data. ``load_curve_annual`` is fetched raw for natgas ID,
# MF ratios, and annual params, but the _sb annual is generated by the
# aggregation step. See ``context/code/data/resstock_sb_annual_load_curves.md``.
