...
dev-setup: aws
    ...
    aws ssm send-command \
        ...
        --parameters 'commands=[
            "bash -c \"set -eu; if ! command -v just >/dev/null 2>&1; then curl --proto =https --tlsv1.2 -sSf https://just.systems/install.sh | bash -s -- --to /usr/local/bin; fi; if ! command -v aws >/dev/null 2>&1; then apt-get update && apt-get install -y awscli; fi; mkdir -p /ebs/home /ebs/shared; chmod 755 /ebs/home; chmod 777 /ebs/shared\""
        ]' \
...
dev-login: aws
    ...
    aws ssm send-command \
        ...
        --document-name "AWS-RunShellScript" \
        --parameters "commands=[
            'bash -c \"set -eu; LINUX_USERNAME=\\\"$LINUX_USERNAME\\\"; usermod -aG sudo \\\"\\\$LINUX_USERNAME\\\" 2>/dev/null || true; echo \\\"\\\$LINUX_USERNAME ALL=(ALL) NOPASSWD:ALL\\\" > /etc/sudoers.d/\\\$LINUX_USERNAME; chmod 440 /etc/sudoers.d/\\\$LINUX_USERNAME; if ! command -v uv >/dev/null 2>&1; then curl -LsSf https://astral.sh/uv/install.sh | sh && cp /root/.cargo/bin/uv /usr/local/bin/uv 2>/dev/null || true && chmod +x /usr/local/bin/uv; fi; if ! grep -q \\\"^user_allow_other\\\" /etc/fuse.conf; then sed -i \\\"s/#user_allow_other/user_allow_other/\\\" /etc/fuse.conf || echo \\\"user_allow_other\\\" >> /etc/fuse.conf; fi; if [ ! -d /data.sb ]; then mkdir -p /data.sb; chmod 755 /data.sb; fi; if ! grep -q \\\"data.sb /data.sb\\\" /etc/fstab; then echo \\\"data.sb /data.sb fuse.s3fs _netdev,allow_other,use_cache=/tmp/s3fs-cache,iam_role=auto,umask=0002,use_path_request_style,endpoint=us-west-2,url=https://s3.us-west-2.amazonaws.com 0 0\\\" >> /etc/fstab; else sed -i \\\"s|data.sb /data.sb.*|data.sb /data.sb fuse.s3fs _netdev,allow_other,use_cache=/tmp/s3fs-cache,iam_role=auto,umask=0002,use_path_request_style,endpoint=us-west-2,url=https://s3.us-west-2.amazonaws.com 0 0|\\\" /etc/fstab; fi; mkdir -p /tmp/s3fs-cache; chmod 1777 /tmp/s3fs-cache; if ! mountpoint -q /data.sb; then mount /data.sb 2>&1 || echo \\\"Mount failed, will retry\\\"; sleep 3; if mountpoint -q /data.sb; then echo \\\"S3 mounted successfully\\\"; else echo \\\"S3 mount still not active\\\"; fi; fi\"'
        ]" \
        --output text >/dev/null
    sleep 3
