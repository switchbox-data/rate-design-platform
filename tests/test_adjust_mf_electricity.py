...
def test_get_non_hvac_mf_to_sf_ratios_returns_ratio_for_present_column():
    ...
    meta = pl.DataFrame({
        ...
        FLOOR_AREA_COL: ["1000-1499", "1000-1499", "750-999", "750-999"],  # 1249.5, 874.5
    }).lazy()
...
@patch("utils.pre.adjust_mf_electricity.NON_HVAC_RELATED_ELECTRICITY_COLS", ("out.electricity.ceiling_fan.energy_consumption.kwh",))
@patch("utils.pre.adjust_mf_electricity.HVAC_RELATED_ELECTRICITY_COLS", ())
def test_adjust_mf_electricity_hourly_one_bldg_scales_and_recomputes_total():
    ...
    lf = _minimal_hourly_df_one_non_hvac(
        consumption_col, intensity_col,
        total_consumption=18.0, total_intensity=0.018,
        non_hvac_val=1.0, non_hvac_intensity_val=0.001,
        n_rows=2,
    )
    ...
    assert df.get_column(HOURLY_TOTAL_ELECTRICITY_CONSUMPTION_COL).to_list() == [17.5, 17.5]
...
def test_adjust_mf_electricity_hourly_one_bldg_ratio_one_leaves_unchanged():
    ...
    assert df.get_column(HOURLY_TOTAL_ELECTRICITY_CONSUMPTION_COL).to_list() == [18.0, 18.0]
...
def test_adjust_mf_electricity_parquet_no_op_when_no_unadjusted_mf(tmp_path: Path):
    ...
    meta = pl.DataFrame({
        BLDG_ID_COL: [1, 2],
        BUILDING_TYPE_RECS_COL: ["Single-Family Detached", "Single-Family Attached"],
        FLOOR_AREA_COL: ["1000-1499", "750-999"],
...
@patch("utils.pre.adjust_mf_electricity.NON_HVAC_RELATED_ELECTRICITY_COLS", ("out.electricity.ceiling_fan.energy_consumption.kwh",))
def test_adjust_mf_electricity_parquet_runs_when_metadata_lacks_mf_column(tmp_path: Path):
