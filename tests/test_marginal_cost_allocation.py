...
from rate_design.ny.hp_rates.scripts.generate_utility_tx_dx_MC import (
...
def test_calculate_pop_weights_normalization():
    ...
    assert abs(sum_w_upstream - 1.0) < 1e-6, f"Upstream weights sum to {sum_w_upstream}, not 1.0"
    assert abs(sum_w_dist - 1.0) < 1e-6, f"Distribution weights sum to {sum_w_dist}, not 1.0"
...
def test_calculate_pop_weights_peak_identification():
    ...
    assert n_upstream_peaks == 100, f"Expected 100 upstream peaks, got {n_upstream_peaks}"
...
def test_calculate_pop_weights_load_weighted():
    ...
    peak_df = result_df.filter(pl.col("is_upstream_peak")).sort("load_mw", descending=True)
