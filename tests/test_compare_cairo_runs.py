...
from utils.post.compare_cairo_runs import _numeric_cols, compare_artifact
...
def _patch_s3_reader(monkeypatch: pytest.MonkeyPatch) -> dict[str, bytes]:
    ...
    monkeypatch.setattr("utils.post.compare_cairo_runs._read_csv_from_s3", _fake_read)
    return store
...
class TestCompareArtifact:
    ...
    def test_small_diff_within_tolerance_passes(
        ...
        chal_df = pl.DataFrame(
            {"bldg_id": [1, 2], "bill": [100.0 + 1e-13, 200.0 - 1e-13]}
        )
    ...
    def test_row_count_mismatch_fails(self, _patch_s3_reader: dict[str, bytes]) -> None:
        store = _patch_s3_reader
