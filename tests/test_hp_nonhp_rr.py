...
from utils.post.compute_hp_nonhp_rr import compute_hp_nonhp_rr
...
def test_compute_hp_nonhp_rr(tmp_path: Path) -> None:
    ...
    assert hp["sum_cross_subsidy"][0] == pytest.approx(40.0)
    assert hp["revenue_requirement"][0] == pytest.approx(360.0)

    assert nonhp["sum_bills"][0] == pytest.approx(200.0)
    assert nonhp["sum_cross_subsidy"][0] == pytest.approx(20.0)
