...
- Load trip schedules: $Trip_{m,d}$, $t_{dep}^{m,d}$, $t_{arr}^{m,d}$, $miles_{m,d}$ for all days $d$ in month $m$
...
The charging control strategy is determined by the current schedule state $S_m^{current}$:

**Default Charging ($S_m^{current} = 1$):**
- EV Max Power: $P^{max}$ (full power charging when home)
- Charging logic: Immediate charging upon arrival until 100% SOC or departure
- No time-based restrictions

**TOU-Adapted Charging ($S_m^{current} = 0$):**

*During Off-Peak Hours ($t \notin H$) and $Home_{m,t} = 1$:*
- EV Max Power: $P^{max}$ (unrestricted charging)
- Priority charging for next-day trip requirements

*During Peak Hours ($t \in H$) and $Home_{m,t} = 1$:*
- EV Max Power: $0$ (no charging during peak hours)
- Rely on pre-charged SOC from off-peak periods

*Calculate Required SOC:*
For each day $d$, calculate minimum SOC needed before departure:
$$
SOC_{req}^{m,d} = \max(SOC^{min}, \frac{miles_{m,d}}{Eff^{EV} \cdot Cap^{battery}} + 0.2)
$$
