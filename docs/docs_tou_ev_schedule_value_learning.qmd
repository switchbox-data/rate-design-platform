...
$$
P_{max}^{m,t} = \begin{cases}
P^{hardware} & \text{if } t \in Window_{off}^{m,d} \text{ and } SOC_{m,t} < SOC_{req}^{m,d} \\
...
- Unmet demand theoretically calculated as: $\max(0, kWh_{req}^{m,d} - |Window_{off}^{m,d}| \times 0.25 \times P^{hardware})$
...
- OCHRE Output: $E_{m,t}^{EV}$ (energy consumption), $SOC_{m,t}$ (state of charge), $Q_{m,t}^{unmet}$ (unmet load)
