...
| $N_{trips}^{m,d}$ | Parameter | Number of trips on day $d$ in month $m$ (from BSF trip schedule) | - | M × D |
| $t_{dep}^{m,d,i}$ | Parameter | Departure time for trip $i$ on day $d$ in month $m$ (integer hours from BSF) | hour | M × D × I |
| $t_{arr}^{m,d,i}$ | Parameter | Arrival time for trip $i$ on day $d$ in month $m$ (integer hours from BSF) | hour | M × D × I |
| $miles^{m,d,i}$ | Parameter | Miles driven for trip $i$ on day $d$ in month $m$ (from BSF trip schedule) | miles | M × D × I |
| $t_{dep}^{first,m,d}$ | Parameter | First departure time on day $d$ in month $m$ (min of all $t_{dep}^{m,d,i}$) | hour | M × D |
| $t_{arr}^{last,m,d}$ | Parameter | Final arrival time on day $d$ in month $m$ (max of all $t_{arr}^{m,d,i}$) | hour | M × D |
| $miles_{total}^{m,d}$ | Parameter | Total daily miles on day $d$ in month $m$ ($\sum_i miles^{m,d,i}$) | miles | M × D |
...
| $\beta$   | Parameter | Monetization factor for range anxiety penalty (household-specific) | $/kWh    | 1 × 1     |
| $Cap^{battery}$ | Parameter | EV battery energy capacity (total kWh storage) | kWh | 1 × 1 |
| $Eff^{EV}$ | Parameter | EV efficiency (miles per kWh) | miles/kWh | 1 × 1 |
| $P^{hardware}$ | Parameter | Physical maximum charging power capacity of home charging equipment | kW | 1 × 1 |
| $P^{max}$ | Parameter | Controllable upper limit for charging power (≤ $P^{hardware}$) | kW | 1 × 1 |
| $SOC^{min}$ | Parameter | Minimum comfortable SOC threshold | - | 1 × 1 |
| $SOC_{critical}^{m,d}$ | Parameter | Critical SOC threshold triggering emergency charging ($SOC_{req}^{m,d} - 0.05$) | - | M × D |
| $P^{emergency}$ | Parameter | Emergency charging power during peak hours | kW | 1 × 1 |
| **Derived Charging Variables** |
| $Window_{avail}^{m,d}$ | Variable | Available charging time periods for day $d$ in month $m$ | - | M × D |
| $Window_{off}^{m,d}$ | Variable | Off-peak charging time periods for day $d$ in month $m$ | - | M × D |
| $Window_{peak}^{m,d}$ | Variable | Peak charging time periods for day $d$ in month $m$ | - | M × D |
| $kWh_{req}^{m,d}$ | Variable | Required charging energy for day $d$ in month $m$ | kWh | M × D |
| $P_{required}^{m,d}$ | Variable | Required charging power for day $d$ in month $m$ | kW | M × D |
| **Learning Parameters** |
...
| $P_{m,t}^{charge}$ | Variable | EV charging power at time $t$ in month $m$ | kW | M × T |
...
0 & \text{if } t_{dep}^{first,m,d} \leq hour(t) < t_{arr}^{last,m,d} \text{ for day } d \text{ containing time } t \\
1 & \text{otherwise (vehicle is home and available for charging)}
\end{cases}
$$

Where $hour(t)$ converts 15-minute time interval $t$ to integer hour.
...
$$
P_{max}^{m,t} = \begin{cases}
P^{max} & \text{if } t \in Window_{off}^{m,d} \text{ and sufficient charging needed} \\
0 & \text{if } t \in Window_{peak}^{m,d} \text{ (no peak charging)} \\
P^{emergency} & \text{if } t \in Window_{peak}^{m,d} \text{ and } SOC_{m,t} < SOC_{critical} \\
0 & \text{if } Home_{m,t} = 0 \text{ (vehicle away)}
\end{cases}
$$

Where:
- $SOC_{critical} = SOC_{req}^{m,d} - 0.05$ (5% below required SOC triggers emergency charging)
- $P^{emergency} = \min(P^{max}, \frac{kWh_{remaining}}{hours_{until\_departure}})$ (minimum power to meet departure requirement)

*Step 5: Charging Priority Algorithm*

Within off-peak periods, prioritize charging to minimize cost while ensuring range requirements:

1. **Primary Goal**: Achieve $SOC_{req}^{m,d}$ using only off-peak hours if possible
2. **Secondary Goal**: If off-peak hours insufficient, use minimum peak hours necessary
3. **Charging Rate**: Use available charging hours to determine required power:

$$
P_{required}^{m,d} = \frac{kWh_{req}^{m,d}}{|Window_{off}^{m,d}| \times 0.25}
$$

Where $|Window_{off}^{m,d}| \times 0.25$ converts count of 15-minute intervals to hours.

**Key Distinctions:**

**Power Capacity vs. Energy Capacity:**
- **$P^{hardware}$** is physical charging **power capacity** (kW) - maximum possible charging power
  - Example: Level 2 home charger hardware = 7.2 kW maximum capability
- **$Cap^{battery}$** is energy **capacity** (kWh) - how much energy the battery stores
  - Example: Tesla Model 3 = 75 kWh battery energy capacity

**Controllable vs. Physical Limits:**
- **$P^{hardware}$** is the fixed physical limit (cannot exceed this)
- **$P^{max}$** is our controllable upper limit ($0 \leq P^{max} \leq P^{hardware}$)
- In OCHRE: "EV Max Power (kW)" control signal sets $P^{max}$ dynamically

**Example:** If $P^{hardware} = 7.2$ kW:
- Default charging: Set $P^{max} = 7.2$ kW (use full hardware capacity)
- TOU off-peak: Set $P^{max} = 7.2$ kW (fast charging when cheap)
- TOU peak: Set $P^{max} = 0$ kW (no charging when expensive)
- Emergency: Set $P^{max} = 3.6$ kW (limited charging if critically needed)

*Step 6: SOC Update Logic*

The state of charge is updated by OCHRE based on:
- Energy consumed during trips: $SOC_{post\_trip} = SOC_{pre\_trip} - \frac{miles^{trip}}{Eff^{EV} \times Cap^{battery}}$
- Energy added during charging: $SOC_{t+1} = SOC_t + \frac{P_{charge}^{m,t} \times 0.25}{Cap^{battery}}$ (for 15-min intervals)

This TOU-adapted strategy ensures range requirements are met while minimizing charging during expensive peak hours, using BSF trip data to precisely calculate when charging must occur.

## Step 2: Run OCHRE Simulation for Month $m$

**Execute Monthly Simulation for Month $m$:**

- Input: Trip schedules, charging control strategy, home availability
- Output: $P_{m,t}^{charge}$, $SOC_{m,t}$, $Q_{m,t}^{unmet}$

**Calculate Monthly Electricity Bill for Month $m$:**

$$
C_m^{bill} = \sum_{t \in T} P_{m,t}^{charge} \cdot r_{m,t} \cdot \frac{1}{4}
$$

Where the factor $\frac{1}{4}$ converts 15-minute intervals to hours.

## Step 3: Assess Range Anxiety Performance for Month $m$

**Extract Unmet Charging Demand from OCHRE for Month $m$:**

$$
Q_{m,t}^{\text{unmet}} = \text{OCHRE output: "EV Unmet Load (kWh)" at time } t \text{ in month } m
$$

Unmet charging load occurs when the EV cannot achieve required SOC due to insufficient charging time or power limitations under TOU restrictions.

**Calculate Monthly Range Anxiety Penalty for Month $m$:**
