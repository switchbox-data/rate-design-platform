...
# Plot water heating comparison for the specified building
if default_file.exists() and tou_file.exists():
    fig = plot_water_heating_comparison(building_id, base_path, days=days_to_plot)
else:
    print(f"Cannot generate water heating comparison plot - missing data files for {building_id}")
```
...
# Plot temperature comparison for the specified building
if default_file.exists() and tou_file.exists():
    fig = plot_temperature_comparison(building_id, base_path, days=days_to_plot)
else:
    print(f"Cannot generate temperature comparison plot - missing data files for {building_id}")
```
...
# Plot monthly bills for the specified building
if monthly_file.exists():
    fig = plot_monthly_bills(building_id, base_path)
else:
    print(f"Cannot generate monthly bills plot - missing data files for {building_id}")
```
...
#| echo: false
#| tbl-cap: "Monthly Decision-Making Results and Financial Performance"

# Load and display monthly results as a table
if monthly_file.exists():
    try:
        monthly_results = load_monthly_results(building_id, base_path)

        # Round numerical columns for better display
        display_df = monthly_results.copy()
        numerical_cols = ['bill', 'comfort_penalty', 'realized_savings', 'unrealized_savings', 'total_cost']
        ...
        display_columns = [
            'Month-Year', 'Schedule State', 'Monthly Bill ($)',
            'Comfort Penalty ($)', 'Total Cost ($)', 'Decision',
            'Realized Savings ($)', 'Unrealized Savings ($)'
        ]
        ...
        print(f"\n### Summary Statistics for {building_id}")
        print(f"- **Total months analyzed**: {len(monthly_results)}")
        print(f"- **Average monthly bill**: ${monthly_results['bill'].mean():.2f}")
        print(f"- **Total realized savings**: ${monthly_results['realized_savings'].sum():.2f}")
        print(f"- **Months on TOU schedule**: {(monthly_results['current_state'] == 'tou').sum()}")
        print(f"- **Total switching decisions**: {(monthly_results['switching_decision'] == 'switch').sum()}")

    except Exception as e:
        print(f"Error loading monthly results: {e}")
else:
    print(f"Monthly results file not found for {building_id}")
```
