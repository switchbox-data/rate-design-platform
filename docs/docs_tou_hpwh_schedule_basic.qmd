...
# File paths - use absolute path from workspace root
import os
base_path = Path("/workspaces/rate-design-platform/rate_design_platform")
default_file = base_path / "outputs" / "default_simulation" / "bldg0000072-up03_default.csv"
tou_file = base_path / "outputs" / "tou_simulation" / "bldg0000072-up03_default_tou.csv"
...
# Plot both datasets (7-day subset)
plt.plot(time_index_7d, default_power_7d, label='Default Schedule', alpha=0.7, linewidth=1)
plt.plot(time_index_7d, tou_power_7d, label='TOU Schedule', alpha=0.7, linewidth=1)

# Customize the plot
plt.xlabel('Time')
plt.ylabel('Water Heating Electric Power (kW)')
plt.title('Water Heating Electric Power Comparison: Default vs TOU Schedule')
plt.legend()
...
# Add some statistics (7-day subset)
default_mean = default_power_7d.mean()
tou_mean = tou_power_7d.mean()
default_max = default_power_7d.max()
tou_max = tou_power_7d.max()

stats_text = f'Default Mean: {default_mean:.3f} kW\nTOU Mean: {tou_mean:.3f} kW\n'
stats_text += f'Default Max: {default_max:.3f} kW\nTOU Max: {tou_max:.3f} kW'
