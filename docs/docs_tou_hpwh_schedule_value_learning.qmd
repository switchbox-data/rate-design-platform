...
| $\lambda$ | Parameter | Experience replay sensitivity parameter | - | 1 × 1 |
...
These functions transform ResStock building characteristics into behavioral adjustment factors:

**Income Factor:**
- $g_{AMI} = (\frac{AMI}{80\%})^{0.6}$ (income factor: higher income households have more resources and education, used in both learning and comfort valuation)

**Building Age Factor:**
- $f_{age} = 1.0 + 0.005 \times \max(0, 2000 - YearBuilt)$ (newer buildings may correlate with tech-savvy households - can be replaced with better proxy in future)

**Household Size Factors:**
- $f_{residents} = 1.0 + 0.1 \times \ln(N_{residents})$ (coordination complexity factor for exploration)
- $g_{residents} = 1.0 + 0.2 \times (N_{residents} - 1)$ (household conflict factor for comfort valuation)

**Water Heater Type Factor:**
- $f_{WH} = \{0.7 \text{ (heat pump)}, 1.0 \text{ (storage)}, 1.5 \text{ (tankless)}\}$ (technology factor affecting ease of experimentation)

**Climate Zone Factor:**
- $g_{climate} = \{0.8 \text{ (zones 1-3)}, 1.0 \text{ (zones 4-5)}, 1.2 \text{ (zones 6-8)}\}$ (climate sensitivity for comfort valuation)
...
$$
\epsilon_m = \sigma(\epsilon_{base} \times f_{AMI} \times \frac{1}{f_{age}} \times \frac{1}{f_{residents}} \times f_{WH} + f_{replay})
$$

Where $\sigma(z) = \frac{1}{1 + e^{-z}}$ is the sigmoid function, ensuring $\epsilon_m \in (0,1)$.

Where $f_{replay}$ is the experience replay factor:

$$
f_{replay} = \lambda \times \max(0, C_{recent}^{total} - E[C^{total}])
$$

- $\lambda$ = experience replay sensitivity parameter (controls how much cost surprises increase exploration)
...
$$
\alpha_m^{learn} = \sigma(\alpha_{base}^{learn} \times g_{AMI} \times \frac{1}{f_{age}})
$$

Where $\sigma(z) = \frac{1}{1 + e^{-z}}$ ensures $\alpha_m^{learn} \in (0,1)$.

Where:

- $\alpha_{base}^{learn}$ = base learning rate for average household (to be calibrated)
- $g_{AMI}$ and $f_{age}$ defined in Household Characteristic Functions section above
...
$$
\beta = \beta_{base} \times g_{AMI} \times g_{residents} \times g_{climate}
$$

Where:

- $\beta_{base}$ = base comfort value for average household (to be calibrated)
- $g_{AMI}$, $g_{residents}$, and $g_{climate}$ defined in Household Characteristic Functions section above

**Example:** For a mid-income family (80% AMI, 3 residents, 1980 building, storage water heater, zone 4):
- Pre-sigmoid exploration: $\epsilon_{base} \times 1.0 \times \frac{1}{1.1} \times \frac{1}{1.11} \times 1.0 + f_{replay} = 0.81\epsilon_{base} + f_{replay}$
- $\epsilon_m = \sigma(0.81\epsilon_{base} + f_{replay})$ (constrained to [0,1])
- Pre-sigmoid learning: $\alpha_{base}^{learn} \times 1.0 \times \frac{1}{1.1} = 0.91\alpha_{base}^{learn}$
- $\alpha_m^{learn} = \sigma(0.91\alpha_{base}^{learn})$ (constrained to [0,1])
- $\beta = \beta_{base} \times 1.0 \times 1.4 \times 1.0 = 1.4\beta_{base}$
...
$$
C_m^{comfort} = \beta \cdot \sum_{t \in T} D_{m,t}^{unmet}
$$
...
Where households with higher income, newer buildings, fewer residents, and smart water heaters have higher baseline exploration rates, plus additional exploration when recent costs exceed expectations, with sigmoid constraints ensuring valid probabilities.

$$
\alpha_m^{learn} = \sigma(\alpha_{base}^{learn} \times g_{AMI} \times \frac{1}{f_{age}})
$$

Where households with higher income and newer buildings learn faster from their experiences, with sigmoid constraints ensuring valid learning rates.
