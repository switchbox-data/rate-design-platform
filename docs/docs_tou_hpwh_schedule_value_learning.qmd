...
-0.2 & \text{zones 1-3 (warm)} \\
0.0 & \text{zones 4-5 (moderate)} \\
0.2 & \text{zones 6-8 (cold)}
\end{cases}$$
Colder climates → higher hot water importance → greater comfort sensitivity. Centered at zero for moderate climates (zones 4-5).
...
$$
\epsilon_m = \text{clamp}_{[0,1]}(\epsilon_{base} + \lambda_1 f_{AMI} + \lambda_2 f_{age} + \lambda_3 f_{residents} + \lambda_4 f_{WH} + \lambda_5 \max(0, C_{recent}^{total} - E[C^{total}]))
$$
...
- Factor values: $f_{AMI} = 0.0$, $f_{age} = \frac{1980-2000}{15} = -1.33$, $f_{residents} = \ln(3) = 1.10$, $f_{WH} = 0.0$, $f_{climate} = 0.0$
