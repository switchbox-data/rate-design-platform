...
$$
\epsilon_m = \epsilon_{base} \times f_{AMI} \times \frac{1}{f_{age}} \times \frac{1}{f_{residents}} \times f_{WH} + f_{replay}
$$
...
- Base exploration: $\epsilon_{base} \times 1.0 \times \frac{1}{1.1} \times \frac{1}{1.11} \times 1.0 = 0.81\epsilon_{base}$
- $\epsilon_m = 0.81\epsilon_{base} + f_{replay}$ (total exploration rate including experience replay)
...
## 2.2 OCHRE-Based Prior Value Initialization

**Realistic Prior Expectations:**

Rather than starting with zero knowledge ($V_1^{default} = V_1^{TOU} = 0$), consumers have prior expectations about schedule performance based on general knowledge, utility communications, and building characteristics. These priors are derived from full-year OCHRE simulations to ensure physical realism.

**Prior Calculation Method:**

Before the learning simulation begins, OCHRE runs complete annual simulations for both schedule types:
...
Where $C_m^{total,default}$ and $C_m^{total,TOU}$ are the monthly total costs (bill + comfort) for each schedule type from the pre-simulation runs.
...
$$
V_1^{default} = P^{default} + \mathcal{N}(0, \sigma_{prior}^2)
$$

$$
V_1^{TOU} = P^{TOU} + \mathcal{N}(0, \sigma_{prior}^2)
$$

Where $\sigma_{prior}$ represents the standard deviation of consumer uncertainty about schedule performance (typically 10-20% of mean costs).

**Implementation Note:** This approach ensures that households begin with realistic expectations derived from their specific building physics while maintaining uncertainty that drives meaningful exploration and learning behavior.
...
This realistic learning process suggests the model should incorporate both household characteristics and experience replay:

$$
\epsilon_m = \epsilon_{base} \times f_{AMI} \times \frac{1}{f_{age}} \times \frac{1}{f_{residents}} \times f_{WH} + f_{replay}
$$

Where households with higher income, newer buildings, fewer residents, and smart water heaters explore more frequently, plus additional exploration when recent costs exceed expectations.
