...
**Income Factor:**
$$f_{AMI} = \left(\frac{AMI}{80\%}\right)^{0.6}$$
Higher income → more experimentation (lower financial risk) and faster learning (education/resources). Power function <1 captures diminishing returns.

**Building Age Factor:**
$$f_{age} = 1.0 + 0.005 \times \max(0, 2000 - YearBuilt)$$
Newer buildings → higher tech adoption. Linear approximation; weak empirical basis.

**Household Size Factor:**
$$f_{residents} = 1.0 + 0.1 \times \ln(N_{residents})$$
More residents → harder coordination, more comfort conflicts. Logarithmic captures diminishing marginal effect.

**Water Heater Technology Factor:**
$$f_{WH} = \begin{cases}
0.7 & \text{heat pump (smart controls)} \\
1.0 & \text{storage (baseline)} \\
1.5 & \text{tankless (complex scheduling)}
\end{cases}$$
...
$$
\alpha_m^{learn} = \sigma(\alpha_{base}^{learn} \times f_{AMI} \times \frac{1}{f_{age}})
$$
