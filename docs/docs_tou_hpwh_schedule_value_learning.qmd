...
- λ and γ coefficients control the strength and direction of household characteristic effects
- Positive λ/γ values increase exploration/learning rates; negative values decrease them
- Suggested starting values: λ₁ = ε_base/10 (income), λ₂ = -ε_base/10 (age), λ₃ = -ε_base/10 (household size), λ₄ = ε_base/10 (technology), λ₅ = ε_base/10 (experience replay); γ₁ = α_base/10 (income), γ₂ = -α_base/10 (age)
...
**Building Age Factor:**
$$f_{age} = \max(0, 2000 - YearBuilt)$$
Newer buildings (f_age = 0) → higher tech adoption. Older buildings have positive values. Linear approximation; weak empirical basis.
...
$$f_{WH} = \begin{cases}
-0.3 & \text{heat pump (smart controls)} \\
0.0 & \text{storage (baseline)} \\
0.5 & \text{tankless (complex scheduling)}
\end{cases}$$

**Climate Zone Factor:**
$$f_{climate} = \begin{cases}
0.8 & \text{zones 1-3 (warm)} \\
1.0 & \text{zones 4-5 (moderate)} \\
1.2 & \text{zones 6-8 (cold)}
\end{cases}$$
...
$$
\epsilon_m = \sigma(\epsilon_{base} + \lambda_1 f_{AMI} + \lambda_2 f_{age} + \lambda_3 f_{residents} + \lambda_4 f_{WH} + \lambda_5 \max(0, C_{recent}^{total} - E[C^{total}]))
$$
...
$$
\alpha_m^{learn} = \sigma(\alpha_{base}^{learn} + \gamma_1 f_{AMI} + \gamma_2 f_{age})
$$
...
$$
\beta = \beta_{base} \times f_{AMI} \times f_{residents} \times f_{climate}
$$
...
- Factor values: $f_{AMI} = 0.0$, $f_{age} = 20$, $f_{residents} = 1.10$, $f_{WH} = 0.0$
- Pre-sigmoid exploration: $\epsilon_{base} + \lambda_1 \times 0.0 + \lambda_2 \times 20 + \lambda_3 \times 1.10 + \lambda_4 \times 0.0 + \lambda_5 \max(0, C_{recent}^{total} - E[C^{total}]) = \epsilon_{base} + 20\lambda_2 + 1.10\lambda_3 + \lambda_5 \max(0, C_{recent}^{total} - E[C^{total}])$
- $\epsilon_m = \sigma(\epsilon_{base} + 20\lambda_2 + 1.10\lambda_3 + \lambda_5 \max(0, C_{recent}^{total} - E[C^{total}]))$ (constrained to [0,1])
- Pre-sigmoid learning: $\alpha_{base}^{learn} + \gamma_1 \times 0.0 + \gamma_2 \times 20 = \alpha_{base}^{learn} + 20\gamma_2$
- $\alpha_m^{learn} = \sigma(\alpha_{base}^{learn} + 20\gamma_2)$ (constrained to [0,1])
...
Where households with higher income, newer buildings, fewer residents, and smart water heaters have higher baseline exploration rates (through positive λ coefficients), plus additional exploration when recent costs exceed expectations, with sigmoid constraints ensuring valid probabilities.
