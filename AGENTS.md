...
| Path                                | Purpose                                                                                                                                                                                                                                                                                                                                                       |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`data/`**                         | Data engineering scripts for ingesting and preparing datasets on S3. Each subdirectory (e.g. `data/cambium/`) holds scripts to fetch, convert, and optionally upload a dataset; run via that directory’s Justfile (e.g. `just prepare` in `data/cambium/`).                                                                                                   |
...
| **`rate_design/{ny,ri}/hp_rates/`** | Scenario entrypoints (`run_scenario.py`), **Justfiles** (primary task interface), and `config/` (tariffs JSON in tariffs/electricity and tariffs/gas, tariff_maps CSV in tariff_maps/electric and tariff_maps/gas, marginal_costs). Large artifacts (buildstock raw/processed, cairo_cases) are git-ignored; sync via S3 or keep local.                       |
...
| **`data/eia/861/`**                 | EIA-861 utility stats (PUDL yearly sales); get_utility_stats_from_eia861.py; Justfile fetch-utility-stats. Invoked from rate_design/ny/hp_rates/Justfile fetch-ny-utility-stats.                                                                                                                                                                              |
| **`data/fred/cpi/`**                | FRED CPI series fetch and ASPE FPL guidelines fetch; Justfile for fetch-cpi and fetch-fpl. FPL output: utils/post/data/fpl_guidelines.yaml.                                                                                                                                                                                                                   |
| **`data/resstock/`**                | ResStock metadata: identify HP customers, heating type, assign_utility_ny (NY). Justfile for resstock-download, resstock-identify-hp-customers, assign-utility-ny. Invoked from rate_design/{ny,ri}/hp_rates/Justfile.                                                                                                                                        |
| **`utils/`**                        | Cross-jurisdiction utilities: preprocessing (`utils/pre/` — tariff mappers, precalc mapping, reweighting, flat tariff creation, marginal-cost allocation `generate_utility_tx_dx_mc`), CAIRO helpers. ResStock metadata (identify HP customers, heating type, assign_utility_ny) moved to `data/resstock/`. All runnable as CLI or imported by `rate_design`. |
...
- **Data**: Versioned inputs are under `rate_design/.../config/tariffs/electricity/` and `.../tariffs/gas/` (JSON) and `.../config/tariff_maps/electric/` and `.../config/tariff_maps/gas/` (CSV). Don’t commit large buildstock or CAIRO case outputs; use `.gitignore` and S3/local paths as in existing Justfiles.
